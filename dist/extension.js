...
var ENDPOINT = String(config.get("llm.endpoint"));
var APIKEY = String(config.get("llm.apikey"));
var MODEL = String(config.get("llm.model"));
function activate(context) {
        ...
        async (message) => {
          switch (message.command) {
            case "user_msg":
              ...
              const config2 = vscode.workspace.getConfiguration("Docstring-GPT");
              ENDPOINT = String(config2.get("llm.endpoint"));
              APIKEY = String(config2.get("llm.apikey"));
              MODEL = String(config2.get("llm.model"));
  ...
  let update = vscode.commands.registerCommand("docstring-gpt.doUpdateContent", (panel, chunkContent) => {
    if (!currentPanel) {
      return;
    }
...
function getWebviewContent(model, image, css, script, messages) {
  var chat_string = "";
  for (let i2 = 1; i2 < messages.length; i2++) {
    ...
    let message = messages[i2].content;
    const chat_bubble = `
...
async function api_call(function_definition) {
  const config2 = vscode.workspace.getConfiguration("Docstring-GPT");
  const system_prompt = String(config2.get("systemPrompt"));
  const format_specs = String(config2.get("documentationSpecification"));
  const temp = Number(config2.get("advanced.temperature"));
  var ENDPOINT2 = String(config2.get("llm.endpoint"));
  var APIKEY2 = String(config2.get("llm.apikey"));
  var MODEL2 = String(config2.get("llm.model"));
  var openai = new openai_default({ apiKey: APIKEY2, baseURL: ENDPOINT2 });
