  ...
  async insert_docstring(generator) {
    if (this.editor) {
      ...
      for await (const chunk of generator) {
        ...
        const newEndPosition = new vscode2.Position(current_selection.start.line + Lines, temp.slice(-1)[0].length);
        const selection_range = new vscode2.Range(current_selection.start, newEndPosition);
...
var ChatWebView = class {
      ...
      switch (message.command) {
        ...
        case "user_msg":
          ...
          for await (const chunk of generator) {
            ...
            vscode3.commands.executeCommand("docstring-gpt.doUpdateContent", this.panel, String(marked(String(this.history[this.history.length - 1].content) + " \u2B24")));
          }
          if (this.panel) {
            this.panel.webview.html = this.getWebviewContent(imageUri, css_Uri, script_Uri);
          }
  ...
  getWebviewContent(image, css, script) {
    ...
    for (let i2 = 1; i2 < this.history.length; i2++) {
      ...
      let message = this.history[i2].content;
      const chat_bubble = `
