  ...
  "node_modules/agentkeepalive/lib/agent.js"(exports2, module2) {
    ...
    var Agent = class extends OriginalAgent {
      constructor(options2) {
        ...
        if (options2.freeSocketTimeout === void 0) {
          options2.freeSocketTimeout = 4e3;
        }
        ...
        if (options2.timeout === void 0) {
          options2.timeout = Math.max(options2.freeSocketTimeout * 2, 8e3);
        }
    ...
    function installListeners(agent, socket, options2) {
...
function createFileFromPath(path, { mtimeMs, size }, filenameOrOptions, options2 = {}) {
  let filename;
...
async function fileFromPath(path, filenameOrOptions, options2) {
...
var getStringFromMaybeBuffer = (x2) => {
  ...
  if (typeof Buffer !== "undefined" && x2 instanceof Buffer) return String(x2);
  return void 0;
};
...
var APIClient = class {
  ...
  buildRequest(options2) {
    ...
    if ("timeout" in options2) validatePositiveInteger("timeout", options2.timeout);
    ...
    if (this.idempotencyHeader && method !== "get") {
      if (!options2.idempotencyKey) options2.idempotencyKey = this.defaultIdempotencyKey();
...
var OpenAI = class extends APIClient {
  ...
  constructor(_b) {
    ...
    super({
      ...
      timeout: (_d = options2.timeout) !== null && _d !== void 0 ? _d : 6e5,
...
var LLM = class {
  ...
  async generate_docstring(function_definition, language) {
...
var Editor = class {
  ...
  indent;
  language_comments;
  constructor(editor, notebook_editor, context) {
    this.editor = editor;
    this.notebook_editor = notebook_editor;
    this.indent = this.editor ? this.calculateIndent() : "";
    const filePath = vscode2.Uri.joinPath(context.extensionUri, "src", "languages.json");
    const languagesJsonData = fs2.readFileSync(filePath.fsPath, "utf8");
...
`;
  ...
  async insert_docstring(generator) {
    if (this.editor) {
      ...
      if (!(this.editor.document.languageId in this.language_comments)) {
        ...
        vscode2.window.showInformationMessage("Language Unsupported for Docstring Generation!");
        return;
      }
      ...
      for await (const chunk of generator) {
        ...
        const newEndPosition = new vscode2.Position(original_selection.start.line + lines.length - 1, lines.slice(-1)[0].length);
        const selection_range = new vscode2.Range(original_selection.start, newEndPosition);
        const formated_docstring = this.enforce_format_docstring(docstring, this.editor.document.languageId);
  ...
  get_num_indent(text) {
    ...
    var count = 0;
    for (var i2 = 1; i2 < text_arr.length; i2++) {
      count = text_arr[i2].split(this.indent).length - 1;
      if (count !== 0) {
        break;
      }
    }
  ...
  enforce_format_docstring(docstring, languageId) {
    ...
    text = text.split("\n").map((line) => wrapLine(line, 80)).join("\n");
...
var paragraph = edit(_paragraph).replace("hr", hr2).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", _tag).getRegex();
...
var blockGfm = {
  ...blockNormal,
  table: gfmTable,
  paragraph: edit(_paragraph).replace("hr", hr2).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", gfmTable).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", _tag).getRegex()
};
var blockPedantic = {
  ...blockNormal,
  ...
  lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
  paragraph: edit(_paragraph).replace("hr", hr2).replace("heading", " *#{1,6} *[^\n]").replace("lheading", lheading).replace("|table", "").replace("blockquote", " {0,3}>").replace("|fences", "").replace("|list", "").replace("|html", "").replace("|tag", "").getRegex()
};
var escape = /^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/;
...
var inlineBreaks = {
  ...inlineGfm,
  br: edit(br2).replace("{2,}", "*").getRegex(),
  ...
  toggle_veiw() {
    if (this.panel) {
      ...
      this.panel = vscode3.window.createWebviewPanel(
        ...
        {
          localResourceRoots: [
            vscode3.Uri.joinPath(this.context.extensionUri, "images"),
            vscode3.Uri.joinPath(this.context.extensionUri, "src", "media")
          ],
          enableScripts: true
        }
      );
...
function activate(context) {
  ...
  const editor = new Editor(vscode4.window.activeTextEditor, vscode4.window.activeNotebookEditor, context);
